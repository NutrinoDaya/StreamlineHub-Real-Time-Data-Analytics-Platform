...
class KafkaEventProducer:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"[SUCCESS]  Connected to Kafka at {self.bootstrap_servers}")
    ...
    def close(self):
        ...
        if self.producer:
            self.producer.flush()
            self.producer.close()
            logger.info("[CONNECTING]  Kafka producer connection closed")
...
class KafkaEventConsumer:
    ...
    def connect(self, topics: List[str], group_id: str = 'streamlinehub-analytics') -> bool:
        ...
        try:
            ...
            logger.info(f"[SUCCESS]  Connected Kafka consumer to topics: {topics}")
            self.stats['start_time'] = time.time()
            return True
        except Exception as e:
            logger.error(f"[ERROR]  Failed to connect Kafka consumer: {e}")
            return False
    ...
    def start_consuming(self):
        ...
        self.running = True
        logger.info("[PROCESSING]  Starting Kafka message consumption...")
    ...
    def close(self):
        ...
        if self.consumer:
            self.consumer.close()
            logger.info("[CONNECTING]  Kafka consumer connection closed")
...
class KafkaManager:
    ...
    async def initialize(self, websocket_manager=None):
        ...
        logger.info("[STARTING]  Kafka manager initialized successfully")
    ...
    def shutdown(self):
        ...
        logger.info("[PROCESSING]  Shutting down Kafka manager...")
        ...
        logger.info("[SUCCESS]  Kafka manager shutdown complete")
...
async def init_kafka_manager(bootstrap_servers: str = 'localhost:9092', websocket_manager=None):
    ...
    try:
        ...
        logger.info("[SUCCESS]  Kafka integration initialized successfully")
        return kafka_manager
    except Exception as e:
        logger.error(f"[ERROR]  Failed to initialize Kafka integration: {e}")
        kafka_manager = None
        raise
