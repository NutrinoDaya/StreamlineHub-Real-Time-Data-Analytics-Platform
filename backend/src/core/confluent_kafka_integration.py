...
# Configure file-based logging for Kafka integration
...
class ConfluentKafkaProducer:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"[SUCCESS]  Confluent Kafka Producer connected to {self.bootstrap_servers}")
            self.stats['start_time'] = time.time()
            return True
            
        except Exception as e:
            logger.error(f"[ERROR]  Failed to connect Confluent Kafka Producer: {e}")
            return False
...
class ConfluentKafkaConsumer:
    ...
    def connect(self, topics: List[str]) -> bool:
        ...
        try:
            ...
            logger.info(f"[SUCCESS]  Confluent Kafka Consumer connected to {topics}")
            self.stats['start_time'] = time.time()
            return True
            
        except Exception as e:
            logger.error(f"[ERROR]  Failed to connect Confluent Kafka Consumer: {e}")
            return False
    ...
    async def initialize(self, websocket_manager=None):
        ...
        try:
            ...
            try:
                ...
                # Detect environment and use appropriate Redis connection
                redis_host = 'redis' if os.path.exists('/.dockerenv') else 'localhost'
                redis_port = 6379 if os.path.exists('/.dockerenv') else 16379
                redis_url = f"redis://:redis_secret@{redis_host}:{redis_port}"
                buffer_initialized = await initialize_data_buffer(redis_url)
                if buffer_initialized:
                    logger.info("[SUCCESS]  Redis data buffer initialized")
                else:
                    logger.warning("[WARNING]   Redis buffer initialization failed, using file fallback")
                ...
                if etl_initialized:
                    logger.info("[SUCCESS]  ETL processor initialized")
                
                # Initialize metrics collector
                metrics_collector = get_metrics_collector()
                metrics_initialized = await metrics_collector.initialize()
                if metrics_initialized:
                    logger.info("[SUCCESS]  Metrics collector initialized")
                
            except Exception as etl_error:
                logger.warning(f"[WARNING]   ETL components initialization failed: {etl_error}")
            ...
            logger.info("[SUCCESS]  Kafka Manager with ETL pipeline initialized successfully")
            return True
            
        except Exception as e:
            logger.error(f"[ERROR]  Failed to initialize Kafka Manager: {e}")
            return False
    ...
    def _handle_consumed_message(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            try:
                ...
                # Use basic metrics calculation (synchronous) - detailed metrics calculated in API layer
                try:
                    # Extract real revenue from transaction events
                    revenue_per_minute = 0.0
                    if processed_event.get('event_type') == 'transaction_completed' and processed_event.get('amount'):
                        revenue_per_minute = float(processed_event.get('amount', 0))
                    
                    # Get basic metrics from Redis counters
                    active_users = int(redis_client.hget('metrics:events', 'active_users') or 0)
                    total_events = int(redis_client.hget('metrics:events', 'total') or 0)
                    
                    metrics = {
                        'timestamp': now.isoformat(),
                        'active_users': max(active_users, total_events),  # Use total events if active users not set
                        'events_per_second': consumer_rate,
                        'revenue_per_minute': revenue_per_minute,
                        'conversion_rate': 0.0,  # Calculated in analytics API from Bronze data
                        'avg_session_duration': 0.0,  # Calculated in analytics API from Bronze data
                        'bounce_rate': 0.0  # Calculated in analytics API from Bronze data
                    }
                    
                except Exception as calc_error:
                    logger.warning(f"Failed to calculate basic metrics: {calc_error}")
                    # Minimal fallback
                    metrics = {
                        'timestamp': now.isoformat(),
                        'active_users': 0,
    ...
    async def _add_to_buffer_and_process(self, data_buffer, event: Dict[str, Any]):
        ...
        try:
            ...
            if buffer_size >= threshold:
                logger.info(f"[PROCESSING]  Buffer threshold reached ({buffer_size}/{threshold}), triggering ETL processing")
                
                # Flush buffer and process events
                events_to_process = await data_buffer.flush_buffer()
                
                if events_to_process:
                    # Get Spark ETL processor and process the batch
                    from src.processing.spark_etl_processor import get_spark_etl_processor
                    ...
                    spark_processor = get_spark_etl_processor(etl_config)
                    if spark_processor:
                        if not spark_processor.is_initialized:
                            await spark_processor.initialize()
                        
                        processing_result = await spark_processor.process_kafka_events_to_bronze(events_to_process)
                        logger.info(f"🔥 Spark ETL processing result: {processing_result['status']} - {processing_result.get('events_processed', 0)} events")
                    else:
                        logger.error("Spark ETL processor not available")
