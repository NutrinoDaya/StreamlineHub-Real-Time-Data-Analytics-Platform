...
class RedisDataBuffer:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"[SUCCESS]  Redis buffer connected to {self.redis_url}")
            return True
    ...
    async def add_event(self, event: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if current_size >= self.batch_threshold:
                logger.info(f"[PROCESSING]  Buffer threshold reached ({current_size} events), triggering flush")
                return True
    ...
    async def flush_buffer(self, batch_size: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if events:
                ...
                logger.info(f"[SUCCESS]  Flushed {len(events)} events from buffer")
    ...
    async def close(self):
        ...
        if self.redis_client:
            ...
            logger.info("[CONNECTING]  Redis buffer connection closed")
...
async def initialize_data_buffer(redis_url: str = "redis://:redis_secret@redis:6379") -> bool:
