...
class ETLProcessor:
    ...
    def _register_default_processors(self):
        ...
        def generate_stats(events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            if events:
                ...
                logger.info(f"[ANALYZING]  Processed {len(events)} events: {dict(event_types)}")
        ...
        logger.info(f"[LOGGING]  Registered {len(self.preprocessors + self.processors + self.postprocessors)} processors")
    ...
    async def process_events(self, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[STARTING]  Starting ETL job {job_id} with {len(events)} events")
            ...
            logger.info(f"[SUCCESS]  ETL job {job_id} completed in {processing_time:.2f}s")
    ...
    async def shutdown(self):
        ...
        self.is_running = False
        logger.info("[CONNECTING]  ETL Processor shutdown complete")
