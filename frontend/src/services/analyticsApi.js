...
export const analyticsApi = {
  ...
  async getTransactionSummary() {
    const response = await api.get('/api/v1/analytics/transaction-summary');
    return response.data;
  },

  // Pipeline health overview
  async getPipelineHealth() {
    const response = await api.get('/api/v1/analytics/pipeline-health');
    return response.data;
  },
  ...
  // Time-filtered aggregations with insertionTime
  async getTimeFilteredAggregations(startTime, endTime, aggregationType = 'all') {
    const response = await api.get('/api/v1/analytics/aggregations/time-filtered', {
      params: {
  ...
  // Enhanced aggregation endpoints with time filtering
  async getCustomerActionsAggregationsFiltered(startTime = null, endTime = null) {
    const params = {};
    if (startTime !== null) params.start_time = startTime;
    if (endTime !== null) params.end_time = endTime;
    
    const response = await api.get('/api/v1/analytics/aggregations/customer-actions', { params });
    return response.data;
  },

  async getRevenueHourlyAggregationsFiltered(startTime = null, endTime = null) {
    const params = {};
    if (startTime !== null) params.start_time = startTime;
    if (endTime !== null) params.end_time = endTime;
    
    const response = await api.get('/api/v1/analytics/aggregations/revenue-hourly', { params });
    return response.data;
  },

  async getSystemMetricsAggregationsFiltered(startTime = null, endTime = null) {
    const params = {};
    if (startTime !== null) params.start_time = startTime;
    if (endTime !== null) params.end_time = endTime;
    
    const response = await api.get('/api/v1/analytics/aggregations/system-metrics', { params });
    return response.data;
  }
};
