...
const HighThroughputDashboard = () => {
  ...
  const getEventDescription = (event) => {
    switch (event.event_type) {
      case 'customer_behavior':
        return `Customer ${event.user_id || event.customer_id}: ${event.action}${event.value ? ` ($${event.value})` : ''}`;
      case 'customer_registration':
        return `New customer: ${event.full_name || event.user_id || event.customer_id} (${event.segment || 'standard'})`;
      case 'customer_profile_update':
        return `Profile update for customer ${event.user_id || event.customer_id} (${event.updated_fields?.length || 0} fields)`;
      case 'transaction':
      case 'transaction_completed':
        return `${event.transaction_id}: $${event.amount || event.total_amount} (${event.status || event.transaction_status || 'completed'})`;
      case 'analytics_metric':
      case 'system_metric':
        return `${event.metric_name || event.service_name}: ${event.value || event.metric_value} ${event.unit || ''}`;
      default:
